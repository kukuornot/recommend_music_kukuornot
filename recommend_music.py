...
# 1. Gemini 클라이언트 초기화
try:
    # 배포 환경(st.secrets)과 로컬 환경(os.getenv) 모두 지원
...
def get_recommendation(age: int, preferred_genre: str, language_choice: str):
    ...
    if language_choice == "선택 안 함":
        language_instruction = "추천 이유를 가장 적절하다고 판단되는 언어(예: 한국어, 영어)로 작성해 주세요."
        output_language = "자유"
    else:
    ...
    try:
        # 할당량이 넉넉하고 안정적인 gemini-1.5-flash 모델로 변경
...
if st.button("음악 추천받기 🌟"):
    with st.spinner("Gemini가 당신을 위한 새로운 음악을 고르는 중입니다..."):
        ...
        if "error" in data:
            ...
            st.info("💡 다른 곡을 원하시면 버튼을 다시 눌러보세요!")
